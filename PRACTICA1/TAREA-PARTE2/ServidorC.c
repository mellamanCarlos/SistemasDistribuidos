...
int main(int argc, char const *argv[])
{
    ...
    while ((read_size = recv(clientSocket, mensajeCliente, 2000, 0)) > 0)
    {
        ...
        snprintf(respuesta, sizeof(respuesta), "%d", atoi(mensajeCliente) + 1);

        // Send the message back to client
        write(clientSocket, respuesta, strlen(respuesta) + 1); // El +1 se agregar para que al final de la cadena se agregue el carácter nulo  ('\0')
    }
